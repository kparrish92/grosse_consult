...
session_all %>% 
  ...
  theme_minimal() +
  scale_color_manual(values=c("#56B4E9", "red", "#E69F00")) +
  ylab("Duration (ms)") + xlab("Relative Intensity") +
  theme(
    strip.background = element_rect(
      color="black", fill="white", size=1.5, linetype="solid"
    )) + 
...
session_all %>% 
  ...
  theme(
    ...
    ylim(0, 100) +
  xlim(-1, 45) +
  labs(caption = "Figure 13: Duration as a function of relative intensity in the all groups in sessions 1, 5 and 6")
```
...
session_all %>% 
  filter(session != 2) %>% 
  ggplot(aes(y = duration, x = wmrv, color = category)) + 
  geom_jitter(alpha = .5, width = .2) +
  theme_minimal() +
  scale_color_manual(values=c("#56B4E9", "red", "#E69F00")) +
  ylab("Duration (ms)") + xlab("WMRV") +
  theme(
    strip.background = element_rect(
      color="black", fill="white", size=1.5, linetype="solid"
    )) + 
    ylim(0, 100) +
  xlim(-1, 45) +
  labs(caption = "Figure 14: Duration as a function of wmrv in the all groups in sessions 1, 5 and 6")
```